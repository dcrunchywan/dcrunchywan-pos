  ...
  function updateUI() { 
    ...
    cart.forEach((c, i) => { totalBelanjaGlobal += (c.harga * c.qty); 
      cartDiv.innerHTML += `<div class="card p-2 mb-1 border-0 bg-light extra-small"><div class="cart-item-row"><div class="cart-item-name"><span class="text-uppercase">${c.item}</span></div><div class="cart-item-controls"><span class="text-muted text-nowrap me-1">@${c.harga.toLocaleString('id-ID')}</span><div class="qty-stepper"><button type="button" class="qty-step-btn" onclick="ubahQtyTombol(${i}, -1)">−</button><input type="number" class="form-control form-control-sm qty-control" value="${c.qty}" onchange="ubahQtyManual(${i}, this.value)"><button type="button" class="qty-step-btn" onclick="ubahQtyTombol(${i}, 1)">+</button></div><button class="btn btn-sm text-danger p-0 px-1 fw-bold cart-delete-btn" onclick="hapusItemKeranjang(${i})">×</button></div></div></div>`;
    }); 
